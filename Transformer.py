...
class Embedding(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # Bert
        """
        for sentence_idx in range(len(input_ids)):
            for idx in range(len(input_ids[sentence_idx])):
                if input_ids[sentence_idx][idx] == 100:
                    embeddings[sentence_idx][idx] = nn.Embedding(1, 1024).float().cuda()
        embeddings += self.pos_embedding(pos_ids)
        """
        return embeddings
