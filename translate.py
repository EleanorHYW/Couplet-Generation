...
def print_summaries(summaries, vocab, output_dir, pattern='%d.txt'):
    ...
    for idx in range(len(summaries)):
        ...
        line = [i2w[tok] for tok in summaries[idx] if tok != vocab[config.end_tok] and tok != vocab[config.pad_tok]]
        fout.write(" ".join(line) + "\n")
        fout.close()


def greedy(model, x, tgt_vocab, max_trg_len=40):
    y = torch.ones(len(x), max_trg_len, dtype=torch.long).cuda() * tgt_vocab[config.pad_tok]
    y[:, 0] = tgt_vocab[config.start_tok]
